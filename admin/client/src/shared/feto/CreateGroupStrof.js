    ...
    for (let s = 1; s < state.StrofaPatternMas.length; s++) {
      if (state.StrofaPatternMas[s] === state.StrofaPatternTypeMas[k]) {
        ...
        for (let i = StartStrofy; i < FinalStrofy; i++) {
          ...
          state.ReturnStrofaPositionMas[s] = state.StrofaPatternTypeMas.length - kk + 1; // в обратном порядке запоминаем, в какую группу kk какая строфа s переместилась (+1 для переноса из ленты в первый блок).
