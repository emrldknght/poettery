...
export function startAnalyzePoem(state) {
  ...
  const firstResult = analyzePoem(poem, commentsDiv, 1, state);
  ...
  if (firstResult.excludedLines.length > 0) {
    ...
    const secondResult = analyzePoem(fixedPoem, commentsDiv, 2, state);
    ...
    checkTonicSystem(secondResult.columnAverages, resultDiv, state);
  } else {
    // Выводим итоговый результат
    ...
    checkTonicSystem(firstResult.columnAverages, resultDiv, state);
  }
}