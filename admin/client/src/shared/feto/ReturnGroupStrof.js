export function ReturnGroupStrof() {
  ...
  for (let s = 2; s < GroupStrof.length + 1; s++) {
...
// объединяем шаблоны и переносим в первый блок шаблонов
  ...
  document.getElementById('ContainerTemplate1').innerHTML = ReturnContainerTemplate.join("<br><br>");
// убирем легенду
...
// заменяем три "<br><br>" на два "<br><br>"
  elem = document.getElementById('ContainerTemplate1').innerHTML;
  elem = elem.replace(/<br><br><br>/g, '<br><br>');
  elem = elem.replace(/<br><\/div><br><br>/g, '</div><br>'); // если остались
  document.getElementById('ContainerTemplate1').innerHTML = elem;
}